...
class WC_Gateway_PayPhone extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_description = __("Credit and debit card payments | Payphone", 'payphone');
        ...
        $this->title = __("PayPhone Application Payments", 'payphone');
...
//        }
    ...
    function process_payment($order_id) {
        ...
        try {
            ...
            $Errors;
            $ErrorDescriptions;
            if (isset($ex['Errors'])|| isset($ex['errors'])) { 
                if (isset($ex['Errors']) ) { $errores=$ex->Errors;
                }else{ $errores=$ex->errors; }
                foreach($errores as $error){   
                    $ErrorDescriptions=[];                 
                    if (isset($error['ErrorDescriptions']) || isset($error['errorDescriptions'])) {
                        if (isset($error['ErrorDescriptions']) ) { $arrayDescriptions= $error->ErrorDescriptions; $errorMessage=$error->Message;
                        }else{ $arrayDescriptions= $error->errorDescriptions; $errorMessage=$error->message;}
                       foreach($arrayDescriptions as $descripcion){
                            $ErrorDescriptions[]=$descripcion;
                        }    
                        $Errors[]=Array($errorMessage=>$ErrorDescriptions);                    
                    }else{
                        $Errors[]=Array("Message"=>$errorMessage);
                    }                    
                }
        ...
        if ($order_id != null) {
            ...
            try {
                ...
                if ($response->statusCode == 3) {
                    ...
                    //$order->update_status('completed', 'Modificado por Comercio a Completado');
                    wp_redirect($redirect_url);
                }
            } catch (WC_PayPhone_Exception $ex) {
                $redirect_url = ($this->redirect_page_decline_id == "" || $this->redirect_page_decline_id == 0) ? get_site_url() . "/" : get_permalink($this->redirect_page_decline_id);
