...
/*
  ...
  Domain Path: /lang
  @fs_premium_only /modules/user-path/send-user-path.php, /modules/acf/, /modules/advanced-filters/, /modules/columns-renaming/, /modules/formulas/, /modules/custom-columns/, /modules/universal-sheet/, /modules/yoast-seo/, /modules/wpml/, /modules/columns-manager/, /modules/spreadsheet-setup/,  /modules/wp-sheet-editor/inc/integrations/notifier.php,/modules/wp-sheet-editor/inc/integrations/extensions.json,  /inc/integrations/
 */
	...
	class WP_Sheet_Editor_Taxonomy_Terms {
		...
		function notify_wrong_core_version() {
			...
			<div class="notice notice-error">
				<p><?php _e('Please update the WP Sheet Editor plugin and all its extensions to the latest version. The features of the plugin "' . $plugin_data['Name'] . '" will be disabled temporarily because it is the newest version and it conflicts with old versions of other WP Sheet Editor plugins. The features will be enabled automatically after you install the updates.', vgse_taxonomy_terms()->textname); ?></p>
			</div>
			<?php
		}
		...
		function after_core_init() {
			if (version_compare(VGSE()->version, '2.25.3-beta.3') < 0) {
				add_action('admin_notices', array($this, 'notify_wrong_core_version'));
				return;
			}
